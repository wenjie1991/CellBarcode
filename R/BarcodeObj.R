...
#' name "BarcodeObj". There can be three elements in a BarcodeObj object,
...
#'
#' \code{messyBc} is a list holds the raw barcodes sequence before filtering,
...
#' diversity of the UMI or
#' mutations in the constant region.
#'
#' \code{cleanBc} is a list holds the barcodes sequence after applying filtering,
#' where each element is a \code{data.table} corresponding to the successive samples.
#' The "cleanBc" element contains 2 columns 1. \code{barcode_seq}: barcode sequence
...
#' \code{messyBc} elements in the BarcodeObj objects will be joined. 
#' For the \code{metadata} element, the \code{sample_name} column, and the \emph{Full outer join} (the record in either BarcodeObj object) will be performed with rownames as the key.
...
#' Access & update sample names in BarcodeObj
#'
#' Get or update sample names in BarcodeObj object.
#'
#' @param barcodeObj A \code{BarcodeObj} object.
...
#' @rdname bc_2df
#' @return A \code{data.table}, with two columns: \code{barcode_seq} and \code{count}.
#' @export
...
#' @rdname bc_2df
#' @return A \code{matrix} with sample by columns and barcode by rows.
#' @export
...
#' @param ... Additional arguments
#' @return Formated BarcodeObj summary text.
#'
#' @examples
#' data(bc_obj)
#'
