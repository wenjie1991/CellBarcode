...
familiar with them, such as `plot`, `print`. We use the convention
...
We can use the `bc_seqQC` function to directly check the sequencing quality of
...
`bc_seqQC` will be a `BarcodeQC` object. Here we get the QC results for 4
samples. 
...
```{r Quality_control_2}
qc_withFilte <- bc_seqQC(fq_filter)
plot(qc_withFilte) 
plot(qc_withFilte[[1]]) 
```
...
bc_subset(bc_obj_umi, sample <- cell_type == "lum")
...
# Verify VDJ barcodes (need to remove)
